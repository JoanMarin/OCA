...
class ResPartner(models.Model):
    ...
    def name_fields_in_vals(self, vals):
        """Override to check if `othernames` is in vals."""
        return super().name_fields_in_vals(vals) or vals.get("othernames")

    def get_extra_default_copy_values(self, order):
        """Override to add '(copy)' suffix to othernames instead of firstname."""
        if order != "first_last":
            return {
                "othernames": (
                    _("%s (copy)", self.othernames) if self.othernames else _("(copy)")
                )
            }
